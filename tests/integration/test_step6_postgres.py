...
def test_step6_paper_lifecycle_and_idempotency():
    ...
    Skipped (not failed) when PostgreSQL is unavailable, exactly like the
    Step 1 integration test. This environment has no PostgreSQL, so this
    test is written but not executed here; report that state honestly.
    """
    ...
    try:
        with conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT 1 FROM pg_constraint WHERE conname = 'paper_positions_idea_id_key'"
                )
                assert cur.fetchone() is not None
                cur.execute(
                    "SELECT 1 FROM pg_constraint WHERE conname = 'paper_fills_position_id_kind_key'"
                )
                assert cur.fetchone() is not None
